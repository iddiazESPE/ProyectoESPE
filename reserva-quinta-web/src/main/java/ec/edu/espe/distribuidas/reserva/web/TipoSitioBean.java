...
public class TipoSitioBean extends BaseBean implements Serializable {
    ...
    private List<TipoSitio> tiposSitio;
    ...
    @PostConstruct
    public void init() {
        this.tiposSitio= this.tipoSitioService.obtenerTodos();
    ...
    public void guardar() {
        try {
            if (this.enAgregar) {
                ...
                FacesUtil.addMessageInfo("Se agregó el Tipo de Sitio: " + this.tipoSitio.getDescripcion());
            } else {
                this.tipoSitioService.modificar(this.tipoSitio);
                FacesUtil.addMessageInfo("Se modific\u00f3 el Tipo de Sitio con c\u00f3digo: " + this.tipoSitio.getCodigo());
            }
        } catch (Exception ex) {
    ...
    public TipoSitio getTipoSitioSel() {
        return tipoSitioSel;
    }
